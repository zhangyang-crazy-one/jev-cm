...
一轮说完，你写过的话和助手的回答会自己留下。之后你直接问现在的问题。对得上的原话自己回来；对不上，这一轮就没有带回的内容，菜单里也不会列出旧对话给你点。选中 `/jev` 后，可以留下当前这段对话，也可以按眼下这段对话用 Jev 打分，只把对得上的原文调进现在这个会话。
...
在 Pi 里用 `/jev` 配置，结果写到 `~/.pi/agent/jev-cm.json`，权限 `0600`。压缩和 `jev-cm` 命令都会读这个文件；同名环境变量优先。命令列表里只有这一行。选中 `/jev` 之后再输入 `memory`，只显示数据库路径和条数。再输入 `remember`，会把当前对话里你和助手的原话写入全局对话库，不经过 Jev。再输入 `recall`，会按当前对话用 Jev 打分，只把对得上、放得下的原文放进当前会话。对不上的不会整段塞进来。
